...
if __name__ == '__main__':
    ...
    meshviewer = MeshViewers(shape=(3, 8))
    ...
    for i, data in enumerate(data_loader) :
        if args.split_term == 'lgtddxo' :
            if i not in [7, 9, 10, 14, 16, 22, 23, 27] :
                continue
        elif args.split_term == 'lgtddxi' :
            if i not in [1, 2, 4, 6, 11, 15, 18, 19] :
                continue
        print(data.label)
        data = data.to(device)
        # print(cnt, data.y)
